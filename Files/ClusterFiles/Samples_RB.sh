...
fileFolder=DPS_MultP #
let expName=50 #

let simulationLength=36000 #
let randomSeed=${SGE_TASK_ID}

let partType=3  # 1 - NPS, 2 - SPS, 3 - DPS, 4 - CPS
let errorType=1 # 0 - Homogeneous, 1 - Heterogeneous

# DPS
let initialP=45 # Initial partition length
let gain=20     # Gain 
let alpha=40 # Alpha
# CPS
let epsilon=0 # Epsilon
let memFac=25 # Memory factor

# Loop function 
let transferType=1 # 0 - Direct transfer, 1 - Indirect transfer
homeSourceDistance="1.0" #
